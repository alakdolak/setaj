...
@section('content')
    ...
    <form action="{{route('doEditCitizen', ['id' => $project->id])}}" method="post" enctype="multipart/form-data">
        ...
        <center class="col-xs-12" style="margin-top: 70px">
            ...
            <input value="{{$project->point}}" type="number" name="point" required min="0">

            <h5>دسته پروژه</h5>
            <select name="tagId">
                @foreach($tags as $tag)
                    @if($tag->id == $project->tag_id)
